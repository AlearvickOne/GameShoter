...
public class AmmoForwardToPoint : StructsSave
{
    ...
    private void FireBulletForwardParameters(int i, int weaponIndex)
    {
        ...
        float speed = default;
        speed += (Time.deltaTime * 10) / Vector3.Distance(_weaponDulo[weaponIndex].position, _firePoint);
        Vector3 pos = Vector3.Lerp(_weaponDulo[weaponIndex].position, _firePoint, speed);
